...
///   Set.order_of_key(k)   returns an integer equal to the number of elements
...
template <typename T> struct segTreeLazy {
  ...
  inline void range_update(int posl, int posr, T val) {
    range_update(1, 1, n, posl, posr, val);
  }
  void range_update(int node, int l, int r, int posl, int posr, T val) {
    push_update(node, l, r);
    if (posl > r || posr < l || l > r)
      return;
    if (l >= posl && r <= posr) {
      Tree[node].lazy += val;
      push_update(node, l, r);
  ...
  T query(int node, int l, int r, int posl, int posr) {
    ...
    int mid = (l + r) >> 1;
    return query(node * 2, l, mid, posl, posr) +
           query(node * 2 + 1, mid + 1, r, posl, posr);
  }
};
