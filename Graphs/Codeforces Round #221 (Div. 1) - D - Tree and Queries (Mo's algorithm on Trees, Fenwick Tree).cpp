...
inline void BITupdate(int ind, int val){
    while(ind<=lim){
        BIT[ind]+=val;
        ind += (ind&-ind);
    }
...
inline void Add(int x){
    x=euler[x];
    //if(++nodeCnt[x]==1){
        if(cnt[color[x]]>0)BITupdate(cnt[color[x]],-1);
        cnt[color[x]]++;
        BITupdate(cnt[color[x]],1);
    //}
}
...
inline void Solve(int Case){
    ...
    for(int i=0,v,k; i<m; i++){
        sii(v,k);
        queries[i]=query(i,dTime[v],eTime[v],k);
        queries[i].x=dTime[v]/Div;
    }
    ...
    for(int i=0; i<m; i++){
        ...
        ans[queries[i].i]= BITquery(lim) - BITquery(queries[i].k-1);
    }
