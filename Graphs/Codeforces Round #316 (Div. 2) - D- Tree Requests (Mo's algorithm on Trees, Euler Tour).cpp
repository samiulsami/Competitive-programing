...
struct query{
    int i,l,r,h,x;
    query(){}
    query(int i, int l, int r, int h, int x):i(i),l(l),r(r),h(h),x(x){}
...
inline void Solve(int Case){
    ...
    for(int i=0,x,y,l,r; i<m; i++){
        sii(x,y);
        queries[i] = query(i,dTime[x],eTime[x],y,dTime[x]/Div);
    }
    ...
    for(int i=0; i<m; i++){
        ...
        bool pos=1;
        if((int)__builtin_popcount(lvlVal[queries[i].h])>1)pos=0;
        ans[queries[i].i]=pos;
    }
