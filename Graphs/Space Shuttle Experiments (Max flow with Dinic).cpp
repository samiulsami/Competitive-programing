#include<bits/stdc++.h>
/*
    Problem: LightOJ 1286 - Space Shuttle Experiments
    Verdict: AC
*/
using namespace std;
...
inline void Solve(int Case){
    ...
    FOR(i,n-1){
        si(E[i]);
        sum+=E[i];
        dinic.addEdge(source,i,E[i]);
    }

    FOR(i,m-1){
        si(I[i]);
        dinic.addEdge(i+n,sink,I[i]);
    }
    ...
    FOR(i,n-1){
        ...
        while(q--){
            si(x);
            x--;
            dinic.addEdge(i,x+n,INT_MAX);
        }
    }

    pf("Case %d: %d\n",Case, sum-dinic.maxFlow(source,sink));
}
