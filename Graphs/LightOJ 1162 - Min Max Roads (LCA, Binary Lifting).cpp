...
struct LCA{
    vector<int>lvl;
    vector<vector<int> >Table,maxTable,minTable;
    int n,LOG;
    LCA(int n):n(n),lvl(n+1),LOG(ceil(log2(n))+1){
        Table.assign(LOG, vector<int>(n+1,-1));
        maxTable.assign(LOG, vector<int>(n+1,-1));
        minTable.assign(LOG, vector<int>(n+1,-1));
        dfs(1,-1);
    ...
    pii query(int u, int v){
        ...
        for(int i=LOG-1; i>=0; i--){
            if(Table[i][u]!=Table[i][v]){
                maxx=max({maxx, maxTable[i][u], maxTable[i][v]});
                minn=min({minn, minTable[i][u], minTable[i][v]});
                u=Table[i][u];
                v=Table[i][v];
            }
        }
