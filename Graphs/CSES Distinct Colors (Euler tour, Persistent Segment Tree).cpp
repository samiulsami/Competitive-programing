...
inline void Solve(int Case){
    ...
    build(roots.back(),1,Time);

    map<int,int>prev;
    for(int i=1; i<=Time; i++){
        node *tmp=roots.back();
        if(prev.count(color[euler[i]]))
            tmp=update(tmp,1,Time,prev[color[euler[i]]], 0);
        prev[color[euler[i]]]=i;
        roots.emplace_back(update(tmp,1,Time,i,1));
    }

    for(int i=1; i<=n; i++)pf("%d ",query(roots[eTime[i]],1,Time,dTime[i],eTime[i]));
