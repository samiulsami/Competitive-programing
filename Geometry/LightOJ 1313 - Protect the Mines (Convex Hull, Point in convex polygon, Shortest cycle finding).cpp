...
void solve(int casenum){
	...
	sii(n,m);
	sii(g,P);
	vector<pt>holes(n),mines(m);
	FOR(i,n)sii(holes[i].x,holes[i].y);
	FOR(i,m)sii(mines[i].x,mines[i].y);
	
	auto holehull=convexHull(holes);
	ll ans=0;
	vector<pt>relevant;
	
	for(pt &p:mines){
		if(pointInConvexPolygon(holehull,p)==1)relevant.push_back(p);
		else ans+=g;
	}
	
	if(!relevant.empty()){
		FOR(i,n)FOR(j,n)dist[i][j]=1e8;
		...
		for(int i=0; i<n; i++){
			for(int j=0; j<n; j++){
				...
				bool b=1;
				for(pt &p:relevant){
					if(orientation(holes[i],holes[j],p)!=-1){
						b=0;
						break;
					}
				}
		...
		int cur=1e9;
		for(int i=0; i<n; i++)cur=min(cur,dist[i][i]);
		ans+=(1LL*cur*P);
	}
