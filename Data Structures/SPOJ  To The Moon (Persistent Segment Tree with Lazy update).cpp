...
struct node{
    ll data,lazy;
    node *left,*right;
    node(ll val=0, ll lazy=0, node* left=NULL, node *right=NULL):lazy(lazy),data(val),left(left),right(right){}
};
...
inline node *newNode(ll data=0, ll lazy=0, node *left=NULL, node *right=NULL){
...
node *cloneAndRangeUpdate(node *cur, int l, int r, int posl, int posr, ll val){
    push_update_clone(cur,l,r);
    if(l>posr || r<posl)return cur;
    if(l>=posl && r<=posr){
        node *tmp = newNode(cur->data);
        tmp->data += 1LL*(r-l+1)*(val);
        if(l!=r){
            int mid = (l+r)>>1;
...
inline void Solve(int Case){
    ...
    while(m--){
        ...
        if(c=='C'){
            siii(x,y,z);
            segTree[stCnt+1]=cloneAndRangeUpdate(segTree[stCnt],1,n,x,y,z);
            stCnt++;
        }
        else if(c=='Q'){
            sii(x,y);
            pf("%lld\n",query(segTree[stCnt],1,n,x,y));
        }
        else if(c=='H'){
            siii(x,y,z);
            pf("%lld\n",query(segTree[z],1,n,x,y));
        }
        else if(c=='B'){
            si(x);
            stCnt=x;
