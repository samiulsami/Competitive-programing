...
inline void Add(int x){
    for(pii factor:factors[x]){
        int mxPow=0;
        for(int i=1; i<=20; i++){
            if(powCnt[factor.ff][i]>=1)mxPow=max(mxPow,i);
        }
        if(++powCnt[factor.ff][factor.ss]==1 && factor.ss>mxPow){
            cur = (cur*bigMod(factor.ff,factor.ss-mxPow))%MOD;
        }
    }
}
...
inline void Remove(int x){
    for(pii factor:factors[x]){
        ...
        if(--powCnt[factor.ff][factor.ss]==0 && factor.ss==mxPow){
            int secondMxPow=0;
            for(int i=1; i<=20; i++){
                if(powCnt[factor.ff][i]>=1)secondMxPow=max(secondMxPow,i);
            }
            ll powdiff = factor.ss-secondMxPow;
            ll val = bigMod(factor.ff,powdiff)%MOD;
            ll inv = bigMod(val,MOD-2);
            cur = (cur*inv)%MOD;
        }
    }
}
