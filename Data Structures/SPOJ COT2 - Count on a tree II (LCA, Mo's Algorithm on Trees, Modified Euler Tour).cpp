...
struct query{
    ...
    bool flag;
    query(){}
    query(int i, int l, int r, bool flag, int L):i(i),l(l),r(r),flag(flag),L(L){}
...
inline void Add(int x){
    int u=euler[x];
    if(!vis[u]){
        if(++colorCnt[color[u]]==1)cur++;
        vis[u]=1;
    }
    else{
        if(--colorCnt[color[u]]==0)cur--;
        vis[u]=0;
    }
}
...
inline void Solve(int Case){
    ...
    sii(n,m);
        ...
        for(int i=0; i<m; i++){
            ...
            ans[queries[i].i]=cur;
            if(queries[i].flag)Remove(queries[i].L);
        }
