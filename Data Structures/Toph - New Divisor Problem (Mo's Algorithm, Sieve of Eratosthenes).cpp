...
const int N = 1e5+5;
const int Div=400;
int DivCnt[int(1e6)+5]={0};
int cnt[int(1e6)+5];
int arr[N];
int ans[N];
int distinct[int(1e6)+5];

void sieve(){
    int mx=1e6;
    for(int i=1; i<=mx; i++){
        for(int j=i; j<=mx; j+=i)DivCnt[j]++;
    }
}
...
struct query{
    int l,r,d,i,x;
    query(){}
    query(int i, int l, int r, int d):i(i),l(l),r(r),d(d){}
...
inline void Solve(int Case){
    ...
    for(int i=0; i<m; i++){
        ...
        ans[queries[i].i] = cnt[queries[i].d];
    }
