...
int Search(node *left, node *right, int l, int r, int k){
    if(l==r)return l;
    int val = right->left->data - left->left->data;
    int mid = (l+r)>>1;

    if(val>=k)return Search(left->left,right->left,l,mid,k);
    return Search(left->right,right->right,mid+1,r,k-val);
}

inline void Solve(int Case){
    int n,m;
    sii(n,m);
    map<int,int>mapped,original;
    int cnt=1;
    int x;
    vector<node*>segTree;
    ...
    for(int x:arr){
        x=mapped[x];
        //pf("%d -> %d\n",x,original[x]);
        segTree.push_back(cloneAndUpdate(segTree.back(),1,n,x,1));
    }
    ...
    while(m--){
        ...
        int ans = Search(segTree[l-1],segTree[r],0,n-1,k)+1;
        pf("%d\n",original[ans]);
    }
