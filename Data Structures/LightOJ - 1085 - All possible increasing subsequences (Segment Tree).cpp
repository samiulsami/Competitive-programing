...
void update(int node, int l, int r, int pos, int val){
    ...
    else if(l==pos && r==pos)Tree[node] = (1LL*Tree[node]+val)%MOD;
    else{
        int mid = (l+r)/2;
        if(pos<=mid)update(node*2, l, mid, pos, val);
        else update(node*2+1, mid+1, r, pos, val);
        Tree[node] = (1LL*Tree[node*2] + 1LL*Tree[node*2+1])%MOD;
    }
}

ll query(int node, int l, int r, int posl, int posr){
    if(l>r || l>posr || r<posl)return 0;
    else if(l>=posl && r<=posr)return 1LL*Tree[node];
    int mid = (l+r)/2;
...
inline void Solve(int Case){
    ...
    for(int i=n-1; i>=0; i--){
        int val = mp[arr[i]];
        update(1,1,n,val,(query(1,1,n,val+1,n)+1)%MOD);
    }
