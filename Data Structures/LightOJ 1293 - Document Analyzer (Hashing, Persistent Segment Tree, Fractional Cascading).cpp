...
int Find(node *cur, int l, int r, int mx){
    if(l==r)return l;
    int val = cur->right->data;
    int mid = (l+r)>>1;
    if(val>=mx)return Find(cur->right, mid+1, r, mx);
    return Find(cur->left, l, mid, mx-val);
}
...
inline void Solve(int Case){
    ...
    while(sf("%s",&s)){
        ...
        for(int i=0; ;i++){
            if(s[i]>='a' && s[i]<='z')H = mod(1LL*H*base + 1LL*s[i]);
            else{
                if(H!=0){
                    if(!mp.count(H))mp[H]=sz(mp)+1;
                    arr[++n]=mp[H];
                    H=0;
                }
                if(s[i]=='\0')break;
    ...
    for(int i=1; i<=n; i++){
        node *tmp=roots.back();
        if(Prev[arr[i]]!=-1)tmp=update(tmp,1,n,Prev[arr[i]],0);
        roots.emplace_back(update(tmp,1,n,i,1));
        Prev[arr[i]]=i;
    }
    ...
    for(int i=mx; i<=n; i++){
        if(query(roots[i],1,n,1,i)<mx)continue;
        int x=Find(roots[i],1,n,mx);
        if((r-l+1)>(i-x+1)){
            l=x;
            r=i;
        }
        else if((r-l+1)==(i-x+1) && x<l){
            l=x;
            r=i;
        }
    }
