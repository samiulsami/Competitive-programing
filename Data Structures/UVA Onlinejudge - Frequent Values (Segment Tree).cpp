...
inline void Solve(int Case){
    ...
    while(si(n)){
        ...
        for(int i=1,st,ed; i<=n; i++){
            st=i,ed=i;
            while(ed+1<=n && arr[ed+1]==arr[st])ed++;
            for(int j=st; j<=ed; j++)freq[j]={st,ed};
            i=ed;
            Tree[i+n-1]=ed-st+1;
        }
        ...
        while(q--){
            ...
            subl=freq[l].ss+1;
            subr=freq[r].ff-1;
            if(freq[subl]==freq[subr])ans2=subr-subl+1;
            else ans2=query(subl-1,subr-1);

            if(freq[l]==freq[r])ans1=r-l+1;
            else ans1 = max(r-freq[r].ff+1, freq[l].ss-l+1);
