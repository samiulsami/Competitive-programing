...
bool cmp(pii a, pii b){
    double x,y;
    x=(double)a.xx/(double)a.yy;
    y=(double)b.xx/(double)b.yy;
    return x<y;
}
...
void bruteforce(int n){
    ...
    //cout<<n<<" "<<farey.size()<<endl;
    sort(farey.begin(),farey.end(),cmp);

    //for(auto val:farey)cout<<val.xx<<" "<<val.yy<<endl;

    int val=0;
    for(int i=0; i<farey.size(); i++){
        for(int j=i+2; j<farey.size(); j++){
            int a,b,x,y;
            a=farey[j].xx;
            b=farey[i].yy;
            x=farey[i].xx;
...
void preCompute(){
    for(int i=0; i<1000002; i++)phi[i]=i;
    //phiCum[1]=1;
