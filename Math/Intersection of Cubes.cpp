...
/*
    problem:   LightOJ 1211 - Intersection of Cubes
    verdict:    AC
*/
...
void solve(int cases){
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<2; j++)
            cin>>cubes[i][j].x>>cubes[i][j].y>>cubes[i][j].z;

        mn.x=max(cubes[i][0].x,mn.x);
        mn.y=max(cubes[i][0].y,mn.y);
        mn.z=max(cubes[i][0].z,mn.z);

        mx.x=min(cubes[i][1].x,mx.x);
        mx.y=min(cubes[i][1].y,mx.y);
        mx.z=min(cubes[i][1].z,mx.z);
    }

    int ans=(mx.x-mn.x)*(mx.y-mn.y)*(mx.z-mn.z);
    ans=max(ans,0);
