...
inline void Solve(int Case){
    int n,q,x,ind;
    map<pair<int,int>,int>primesCnt; /// Number of times x^y occurs in the array
    ...
    FOR(i,n){
        ...
        for(pii p:tmp){
            arr[i+1][p.ff]=p.ss;
            for(int j=1; j<=p.ss; j++)primesCnt[{p.ff,j}]++;
        }
    }
    ...
    for(int p:primes){
        int maxPow=0;
        for(int j=1; j<=20; j++){
            if(primesCnt.count({p,j}))
                if(primesCnt[{p,j}]==n)maxPow=j;
        }
        if(maxPow>0)G = (G*bigMod(p,maxPow))%MOD;
    }
    ...
    while(q--){
        ...
        for(pii p:tmp){
            int maxPow=0;
            int cur = arr[ind][p.ff];
            for(int j=1; j<=p.ss; j++){
                if(++primesCnt[{p.ff,j+cur}]==n)maxPow=j;
            }
            if(maxPow>0)G = (G*bigMod(p.ff,maxPow))%MOD;
            arr[ind][p.ff]+=p.ss;
            //dbug(p);
        }
