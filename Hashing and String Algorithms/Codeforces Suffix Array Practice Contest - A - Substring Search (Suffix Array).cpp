...
namespace SuffixArray{
    ...
    inline bool exists(const string &s, const string &x){
        int low=0,high=n-1,mid;
        while(low<=high){
            mid = (low+high)>>1;
            if(x<s.substr(SA[mid],(int)x.size()))high=mid-1;
            else if(x==s.substr(SA[mid],(int)x.size()))return 1;
            else low=mid+1;
        }
        return 0;
    }
