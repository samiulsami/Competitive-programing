...
inline void solve(int caseNum){
	...
	for(int i=0; i<n; i++){
		...
		while(l<=r && !st.count(H.fHash(l,r))){
			palindromes.push_back({l,r});
			st.insert(H.fHash(l,r));
			l++,r--;
		}
	...
	sort(all(palindromes), [&](const auto &L, const auto &R)->bool{
		int lenL = L[1]-L[0]+1;
		int lenR = R[1]-R[0]+1;
		if(L[0]==R[0])return lenL<lenR;
		int lcp = min({SA.lcp(L[0],R[0]), lenL, lenR});
		if(lcp==lenL || lcp==lenR)return lenL<lenR;
		return s[L[0]+lcp]<s[R[0]+lcp];
	});

	//for(auto i:palindromes)cout<<i[0]<<"-"<<i[1]<<" "<<s.substr(i[0],i[1]-i[0]+1)<<"\n";
	vector<ll>total(sz(palindromes),0);
	total[0] = palindromes[0][1] - palindromes[0][0] + 1;
	for(int i=1; i<sz(palindromes); i++)total[i] = total[i-1] + ll(palindromes[i][1] - palindromes[i][0]+1);
	...
	while(q--){
		...
		int low=0,high=sz(palindromes)-1,mid,cur;
		while(low<=high){
			mid = (low+high)>>1;
			if(total[mid]>=l){
				cur=mid;
		...
		if(cur)l -= total[cur-1];
		l = palindromes[cur][0]+l-1;

		string ans;
		while(sz(ans)<reqLen){
			ans.push_back(s[l]);
			if(sz(ans)==reqLen)break;
			l++;
			if(l>palindromes[cur][1]){
				cur++;
				l = palindromes[cur][0];
			}
		}

		assert(sz(ans)==reqLen);
		cout<<ans<<"\n";
	}
}
