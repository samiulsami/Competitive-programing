...
ll f(int i, int j){
    if(i==n){
        return ac.trie[j].terminal || ac.trie[j].next_terminal!=-1;
    }
    ll &ret = dp[i][j];
    if(~ret)return ret;

    if(ac.trie[j].terminal || ac.trie[j].next_terminal!=-1)return ret = 2LL*f(i+1,j);
    ret = f(i+1, ac.getnext(j,'0')) + f(i+1, ac.getnext(j,'1'));
    return ret;
}
