...
struct AhoCorasick{
    ...
    void get(){
        for(int u=0; u<sz(trie); u++){
            for(int i:trie[u].ind){
                for(int j:trie[u].inds)arr[i].push_back(j);
            }
        }
    }
...
inline void Solve(int Case){
    ...
    while(cin>>n){
        ...
        for(int i=0; i<sz(s); i++){
            ...
            while(cur!=-1){
                int lvl = ac.trie[cur].lvl;
                ac.trie[cur].inds.push_back(i-lvl+1);
