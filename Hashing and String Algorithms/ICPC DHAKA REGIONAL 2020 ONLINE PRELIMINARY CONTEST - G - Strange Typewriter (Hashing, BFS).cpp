...
int f(){
    ...
    while(!q.empty()){
        pii top=q.front();
        q.pop();
        if(top.ff>lvl[top.ss])continue;
        int curlen=top.ff;
        int curInd=top.ss;

        for(int i=0; i<sz(hashlen); i++){
            if(curInd+lenArr[i]>n)break;
            if(binary_search(all(hashlen[i]), sHash.getHash(curInd+1, curInd+lenArr[i]))){
                if(curlen+1<lvl[curInd+lenArr[i]]){
                    lvl[curInd+lenArr[i]]=curlen+1;
                    q.push(pii(curlen+1,curInd+lenArr[i]));
                }
            }
        }
    }
