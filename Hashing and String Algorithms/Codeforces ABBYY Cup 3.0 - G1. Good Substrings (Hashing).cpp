#include<bits/stdc++.h>
/// Codeforces ABBYY Cup 3.0 - G1. Good Substrings
using namespace std;
typedef int64_t ll;
...
struct rule{
    ...
    int l,r;
    rule(){}
    rule(Hash H, int l, int r):H(H),l(l),r(r){}
};
...
inline void Solve(int Case){
    ...
    FOR(i,n){
        cin>>s>>l>>r;
        rules.emplace_back(rule(Hash(s,sz(s)), l, r));
    }

    unordered_map<ll,bool>visitedSubstrings;
    visitedSubstrings.max_load_factor(0.25);
    ll tmpHash;
    int ans=0;

    for(int i=1; i<=len; i++){
        for(int j=i; j<=len; j++){
            tmpHash=a.getHash(i,j);
            if(visitedSubstrings.count(tmpHash))continue;
            visitedSubstrings[tmpHash]=1;
            int substrLen = j-i+1;
            ...
            for(rule &R:rules){
                int curLen=R.H.n;
                int occurences=0;
                for(int k=1; k+substrLen-1<=curLen; k++){
                    if(tmpHash==R.H.getHash(k,k+substrLen-1))occurences++;
                    if(occurences>R.r)break;
                }

                if(occurences<R.l || occurences>R.r){
                        possible=0;
                        break;
                }
            }
