...
inline void Solve(int Case){
    ...
    FOR(cntr,cnt){
        ...
        if(sz(ans)<sz(inp))maxMatch = check(ans, inp.substr(0,sz(ans)));
        else maxMatch = check(ans.substr(sz(ans)-sz(inp)), inp);

        for(int i=maxMatch; i<sz(inp); i++)ans.push_back(inp[i]);
    }
    cout<<ans<<endl;
