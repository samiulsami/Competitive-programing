...
inline void Solve(int Case){
    ...
    for(int i=0; i<m; i++){
        cin>>k[i]>>strings[i];
        ac.insert(strings[i],i);
    }
    ac.build();

    int id=0;
    for(int i=0,n=sz(t); i<n; i++){
        ...
        while(cur!=-1){
            indexes[ac.trie[cur].ind].push_back(i);
            cur = ac.trie[cur].next_terminal;
        }
    }

    for(int i=0,ans; i<m; i++){
        if(sz(indexes[i])<k[i])cout<<-1<<endl;
        else{
            int ans=sz(t);
            for(int j=k[i]-1; j<sz(indexes[i]); j++)
                ans=min(ans, indexes[i][j] - indexes[i][j-k[i]+1]);

            cout<<sz(strings[i])+ans<<endl;
        }
    }
}
