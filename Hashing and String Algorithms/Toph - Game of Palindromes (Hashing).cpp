...
const int bases[_hashCnt] = {rand(11887,1<<21)};
...
inline void Solve(int Case){
    ...
    for(int i=1; i<=n; i++){
        low=1,high=min(i,n-i+1);
        mx=1;
        while(low<=high){
            mid=(low+high)/2;
            if(a.singleHash(i-mid+1,i,0)==a.singleReverseHash(i,i+mid-1,0)){
                mx=max(mx,mid);
                low=mid+1;
            }
            else high=mid-1;
        ...
        while(low<=high){
            mid=(low+high)/2;
            if(a.singleHash(i-mid+1,i,0)==a.singleReverseHash(i+1,i+mid,0)){
                mx=max(mx,mid);
                low=mid+1;
            }
            else high=mid-1;
    ...
    double ans = (double)palindromes/((double)((1LL*n*(n+1LL))/2LL));
    pf("%0.3lf\n",ans);
