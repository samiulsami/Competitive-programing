#include<bits/stdc++.h>
/*
    Problem: acmp.ru - PROBLEM #1159 Manacher's Algorithm
    Verdict: AC
*/
using namespace std;
...
const int bases[_hashCnt] = {rand(149,1<<15)};
...
inline void Solve(int Case){
    ...
    for(int i=1,len; i<=n; i++){
        ...
        while(low<=high){
            mid=(low+high)/2;
            if(stHash.singleHash(i-mid+1,i)==stHash.singleReverseHash(i,i+mid-1)){
                low=mid+1;
                len=max(len,mid);
            }
            else high=mid-1;
        ...
        while(low<=high){
            mid=(low+high)/2;
            if(stHash.singleHash(i-mid+1,i)==stHash.singleReverseHash(i+1,i+mid)){
                low=mid+1;
                len=max(len,mid);
            }
            else high=mid-1;
