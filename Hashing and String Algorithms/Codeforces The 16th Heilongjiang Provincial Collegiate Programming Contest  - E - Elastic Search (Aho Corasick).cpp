...
inline void Solve(int Case){
    ...
    for(int i=0; i<n; i++){
        ...
        if(!mp.count(s)){
            mp[s]=1;
            arr2.emplace_back(pair<string,int>(s,x));
        }
    }
    ac.build();
    sort(arr2.begin(),arr2.end(),[&](auto &a, auto &b){return sz(a.first)<sz(b.first);});
    ...
    for(int i=0; i<sz(arr2); i++){
        ...
        int k=arr2[i].second;
        ans[k]=ac.trie[k].cnt;
        for(char c:arr2[i].first){
            id=ac.getnext(id,c);
            int cur;
            if(ac.trie[id].terminal)cur=id;
            else cur=ac.trie[id].next_terminal;
            while(cur!=-1){
                if(cur!=k)ans[k] = max(ans[k], ans[cur]+ac.trie[k].cnt);
                int nxt = ac.trie[cur].next_terminal;
                ac.trie[cur].next_terminal=-1;
                cur=nxt;
            }
        }
        ans1=max(ans1,ans[k]);
