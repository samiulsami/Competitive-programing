    ...
    Hash(const T &s, int n):n(n){
        for(int ind=0; ind<_hashCnt; ind++){
            ...
            for(int i=0,j=n+1; j>=2 && i<n; i++,j--){
                H[ind][i+1] = mod((1LL*H[ind][i]*bases[ind]) + ((ll)s[i]+1LL));
            }
        }
    }
    template<typename T>
...
unordered_map<ll,bool>sampleHashes[1005];
...
inline void Solve(int Case){
    ...
    for(int len=1; len<=m; len++){
        sampleHashes[len].max_load_factor(0.25);
        for(int i=1; i+len-1<=m; i++){
            sampleHashes[len][S.getHash(i,i+len-1)]=1;
        }
    }
    ...
    for(int Q=1; Q<=q; Q++){
        si(k);
        low=1,high=min(m,n-k+1);
        int ans=0;
        while(low<=high){
            mid=(low+high)/2;
            tmpHash = B.getHash(k,k+mid-1);
            if(sampleHashes[mid].count(tmpHash)){
                ans=max(ans,mid);
                low=mid+1;
            }
            else high=mid-1;
