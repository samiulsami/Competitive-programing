#include<bits/stdc++.h>
/// acmp.ru - PROBLEM #1158 Cyclic Shifts
using namespace std;
typedef int64_t ll;
...
const int mods[_hashCnt] = {1000008259,1000004459};
...
bool cmp(int i, int j){
    ...
    if(lastMismatch==INT_MAX)return i<j;
...
inline void Solve(int Case){
    ...
    n = strlen(s);
    vector<int>pos;
    for(int i=0,j=n; i<n; i++,j++)s[j]=s[i],pos.pb(i);
    n+=n;
    s[n]='\0';
    stHash = Hash(s,n);
    ...
    int ind=0;
    bool flag=1;
    for(int i=0,len=n>>1,j; i<len; i++){
        if(pos[i]==0)pf("%d\n",i+1);
        out[ind++]=s[pos[i]+(n>>1)-1];
    }
    out[ind]='\0';
    pf("%s\n",out);
    pf(nl);
