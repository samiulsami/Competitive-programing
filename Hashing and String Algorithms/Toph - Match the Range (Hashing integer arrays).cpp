...
struct Hash{
    ...
    Hash(const T &s, int n):n(n){
        for(int ind=0; ind<_hashCnt; ind++){
            ...
            for(int i=0; i<n; i++){
                H[ind][i+1] = mod((1LL*H[ind][i])+((ll)P[ind][s[i]]));
            }
        }
    }
...
inline void Solve(int Case){
    ...
    while(q--){
        ...
        if((x1>y1) || (x2>y2) || (y1-x1)!=(y2-x2))pf("No\n");
        else if(a.getHash(x1,y1)==a.getHash(x2,y2))pf("Yes\n");
        else pf("No\n");
    }
}
...
int main(){
    for(int i=0; i<_hashCnt; i++){
        P[i].push_back(1);
        for(int j=1; j<N; j++)P[i].push_back(mod(1LL*P[i].back()*bases[i]));
        //for(int j=0; j<=20; j++)dbug(P[i][j]);
    }
    #ifdef fastio
