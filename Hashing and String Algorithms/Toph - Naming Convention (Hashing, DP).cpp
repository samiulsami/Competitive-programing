...
bool f(Hash &h, int i, int cnt){
    if(i>h.n)return cnt>=2;
    int &ret = dp[i][cnt];
    if(!MAX(vis[i][cnt],color))return ret;
    ret=0;
    for(int j=i; j<=h.n; j++){
        if(hashes.count(h.getHash(i,j)))ret|=f(h,j+1,cnt+1);
        if(ret)break;
    }
    return ret;
}
...
inline void Solve(int Case){
    int n;
    cin>>n;
    vector<Hash>strings(n);
    string s;

    FOR(i,n){
        cin>>s;
        strings[i]=Hash(s,sz(s));
        hashes.insert(strings[i].getHash(1,sz(s)));
    }
    ...
    if(!exists)cout<<"No solution"<<endl;
