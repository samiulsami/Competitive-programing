...
inline void Solve(int Case){
    ...
    FOR(i,n){
        si(arr[i+1]);
        if(!vis.count(arr[i+1])) val[arr[i+1]]=rng64();
        else vis[arr[i+1]]=1;
    }

    map<pair<ull,int>,ll>mp;
    mp[{0,0}]=1;
    ll ans=0;
    ull prefixXor=0;
    for(int i=1; i<=n; i++){
        prefixXor^=val[arr[i]];
        bool parity = bool(i&1);
        auto &mpref = mp[{prefixXor,parity}];
        if(mpref>0)ans+=mpref;
        mpref++;
    }
